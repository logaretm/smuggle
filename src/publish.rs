...
pub fn cmd_publish(pkg_dir: &Path, select_all: bool) -> Result<(), String> {
    ...
    if let Some(ws) = workspace::detect_workspace(&pkg_dir) {
        return cmd_publish_workspace(ws, select_all);
    }
...
fn cmd_publish_workspace(ws: workspace::DetectedWorkspace, select_all: bool) -> Result<(), String> {
    ...
    // The root package and private packages are never publishable
    ...
    let selected_indices: Vec<usize> = if select_all {
        ...
        for (i, p) in packages.iter().enumerate() {
            prompt = prompt.item(i, format!("{} @ {}", p.name, p.version), "");
        }
